...
def CompactCNN(input_shape, nb_conv, nb_filters, n_mels, normalize, nb_hidden, dense_units,
               output_shape, activation, dropout, time_res, multiple_segments=False, graph_model=False, input_tensor=None):
    ...
    if n_mels >= 256:
        ...
        poolings = [(2, 4), (4, 5), (4, 9), (4, 10), (4, 4)] #50
        if time_res == 2:
            poolings = [(2, 4), (4, 5), (4, 9), (4, 5), (4, 4)] # 100
        if time_res == 3:
            poolings = [(2, 4), (4, 5), (4, 9), (4, 3), (4, 4)] #150
        if time_res in [4, 5]:
            poolings = [(2, 4), (4, 5), (4, 9), (4, 2), (4, 4)] #200 & 250
        if time_res == 10:
            poolings = [(2, 4), (4, 5), (4, 4), (4, 1), (4, 4)] #500
    elif n_mels >= 96:
        poolings = [(2, 4), (4, 5), (3, 9), (4, 10), (4, 4)] # 50
        if time_res == 2:
            poolings = [(2, 4), (4, 5), (3, 9), (4, 5), (4, 3)] #100
        if time_res == 3:
            poolings = [(2, 4), (4, 5), (3, 9), (4, 3), (4, 3)] #150
        if time_res in [4, 5]:
            poolings = [(2, 4), (4, 5), (3, 9), (4, 2), (4, 3)] #200 & 250
        if time_res == 10:
            poolings = [(2, 4), (4, 5), (3, 9), (4, 1), (4, 3)] # 500
    elif n_mels >= 48:
        poolings = [(2, 4), (4, 5), (3, 9), (2, 10), (4, 4)] # 50
        if time_res == 2:
            poolings = [(2, 4), (4, 5), (3, 9), (2, 5), (4, 3)] #100
        if time_res == 3:
            poolings = [(2, 4), (4, 5), (3, 9), (2, 3), (4, 3)] #150
        if time_res in [4, 5]:
            poolings = [(2, 4), (4, 5), (3, 9), (2, 2), (4, 3)] #200 & 250
        if time_res == 10:
            poolings = [(2, 4), (4, 5), (3, 9), (2, 1), (4, 3)] # 500
    elif n_mels >= 32:
...
if __name__ == "__main__":
    ...
    models = []
    for melbands in ['mel128_log', 'mel96_log', 'mel48_log', 'mel128', 'mel96', 'mel48']:
        for time_red in [10, 2,3,4,5]:
            ...
            if time_red in [2, 4, 5, 10]:
                input_shape = (input_shape[0], (input_shape[1]//time_red), input_shape[2])
            else:
                input_shape = (input_shape[0], (input_shape[1]//time_red)+1, input_shape[2])
            ...
            model = CompactCNN(input_shape, nb_conv = nb_conv_layers, nb_filters= nb_filters, n_mels = input_shape[0],
                                ...
                                dropout = dropout, time_res=time_red)
            model.summary()
            ...
            # START TRAINING
            history = model.fit(data_train[:,:,::time_red,:], classes_train,
                        #validation_split=validation_split,
                        validation_data=(data_val[:,:,::time_red,:], classes_val),
                        verbose=2,
                        batch_size=batch_size,
                        epochs=epochs,
                        callbacks=callbacks+[TestCallback(data_test[:,:,::time_red,:], classes_test, data_val[:,:,::time_red,:], classes_val, data_train[:,:,::time_red,:], classes_train, "time_{}_{}".format(melbands, str(50*time_red)))]
                        )

            model.save_weights(os.path.join(MODELS_PATH, "model_repr_log_time_{}_{}.h5".format(str(50*time_red), melbands)))
