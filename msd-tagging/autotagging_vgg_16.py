...
def batch_block_generator(train_set, id2tag, batch_size=32, bins=96, melbands_use=1):
    block_step = 50000
    n_train = len(train_set)
    randomize = True
    while 1:
        for i in range(0, n_train, block_step):
            npy_train_mtrx_x = os.path.join(MODELS_PATH, 'repr16_{}bin_{}_x.npy'.format(bins,i))
            npy_train_mtrx_y = os.path.join(MODELS_PATH, 'repr16_{}bin_{}_y.npy'.format(bins,i))
            ...
            for j in range(0, len(items_list), batch_size):
                if j+batch_size <= x_block.shape[0]:
                    ...
                    x_batch = x_block[items_in_batch,::melbands_use,:,:]
...
def CompactCNN(input_shape, nb_conv, nb_filters, n_mels, normalize, nb_hidden, dense_units,
    ...
    if n_mels >= 256:
        ...
        poolings = [(2, 4), (4, 5), (4, 9), (4, 10), (4, 4)]
    elif n_mels >= 96:
        poolings = [(2, 4), (4, 5), (3, 9), (4, 10), (4, 3)] #(2, 8), (4, 3)]
    elif n_mels >= 72:
        poolings = [(2, 4), (3, 4), (2, 5), (2, 4), (3, 4)]
    elif n_mels >= 64:
        poolings = [(2, 4), (2, 4), (2, 5), (2, 4), (4, 4)]
    elif n_mels >= 48:
        poolings = [(2, 4), (4, 5), (3, 9), (2, 10), (4, 4)]
    elif n_mels >= 32:
        poolings = [(2, 4), (2, 5), (3, 9), (2, 10), (4, 4)]
    elif n_mels >= 24:
        poolings = [(2, 4), (2, 4), (3, 9), (2, 10), (4, 4)]
    elif n_mels >= 16:
        poolings = [(2, 4), (2, 5), (2, 9), (2, 10), (4, 4)]
    elif n_mels >= 8:
        poolings = [(2, 4), (2, 4), (2, 9), (1, 10), (4, 4)]
...
class TestCallback(keras.callbacks.Callback):
    def __init__(self, test_data, test_classes, val_set, val_classes, model_checkpoint):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        print('Validation:')
        print('Epoch: '+str(epoch)+' ROC-AUC '+str(roc_auc/50)+' PR-AUC '+str(pr_auc/50))
        if roc_auc/50 > self.best:
            self.best= roc_auc/50
...
if __name__ == "__main__":
    ...
    model_checkpoint = os.path.join(MODELS_PATH, "model_vgg_repr_16_{}_{}_best.h5".format(args.bins, args.meluse))
    ...
    history = model.fit_generator(batch_block_generator(train_list, id2tag, batch_size, bins=args.bins, melbands_use=args.meluse),
                         steps_per_epoch = 6300,
                         validation_data=(data_val[:,::args.meluse,:,:], classes_val),
                         epochs=epochs,
                         verbose=2,
                         initial_epoch=past_epochs,
                         callbacks=callbacks+[TestCallback(data_test[:,::args.meluse,:,:], classes_test, data_val[:,::args.meluse,:,:], classes_val, model_checkpoint)]
                         )
    """

    model.load_weights(model_checkpoint)
    test_pred_prob = model.predict(data_test)
    np.save('pred_classes.npy', test_pred_prob)
    """
